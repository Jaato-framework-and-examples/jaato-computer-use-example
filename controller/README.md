...
```
 ...
 │                                                                    │
 │   IPCClient ──► jaato daemon ──► openrouter / gemini-2.5-flash      │
...
The loop is **controller-driven**: each turn the controller pushes a set-of-marks
image + tree, the agent calls exactly one `screen.*` tool, the handler executes
it on the device (act → await settle → recover → re-observe), and the next turn
shows the updated screen. Images reach the model via `send_message(attachments=)`
(tool *results* are text-only), so one device action = one agent turn.
...
- **`.jaato/profiles/a11y-controller.yaml`** — the LLM (jaato profile):
  provider `openrouter`, model `google/gemini-2.5-flash`, key via
...
- **Protocol/policy unit tests** — `pytest tests/` (15 tests: framing, snapshot,
  selector, settle, action, grounding, recovery, annotate).
- **Device-side end-to-end (mock device)** — `python tools/mock_device_smoke.py`:
  ...
  act → settle → re-observe, one-action-per-turn guard, SET_TEXT, audit trail.
- **jaato/LLM side** — `python tools/jaato_connect_smoke.py`: fresh-daemon
  ...
  dispatch by gemini-2.5-flash.
- **Real hardware** — not yet run end-to-end; needs the physical device to dial
  in (the two smokes prove each half independently).
